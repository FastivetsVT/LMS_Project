...
Производится в рамках двух команд. Первая зачисляет студента с указанным идентификатором на курс с указанным идентификатором. Вторая отчисляет указанного студента с указанного курса. Если студента с указанным идентификатором нет, необходимо вывести сообщение: Student with id xx doesn’t exist. Если курса с указанным идентификатором нет, необходимо вывести сообщение: Course with id xx doesn’t exist. В случае, если на курс уже записано больше 12 студентов, должно выводиться сообщение: Course xx cannot have more than 12 students.
...
Создает задачу в рамках указанного курса. Каждый из студентов, обучающихся на курсе, может иметь какую-то оценку (от 0 до 100%) по каждой задаче.
...
Выводит список всех студентов, обучающихся на курсе. В качестве параметра указывается идентификатор курса. Если курса с указанным идентификатором нет, необходимо вывести сообщение: Course with id xx doesn’t exist. Если на курсе не обучается ни одного студента, вместо списка студентов выводить сообщение No students enrolled to the course.
...
Выводит журнал успеваемости (список всех задач курса и список всех оценок студентов по каждой из задач) на экран. В качестве параметра указывается идентификатор курса. Если курса с указанным идентификатором нет, необходимо вывести сообщение: Course with id xx doesn’t exist. Если на курсе не обучается ни одного студента, выводить сообщение No students enrolled to the course. Если курс не содержит задач, выводить сообщение No tasks added yet.
...
Сохраняет журнал успеваемости в указанном файле (формат файла см. выше). В качестве параметра указывается идентификатор курса. Если курса с указанным идентификатором нет, необходимо вывести сообщение: Course with id xx doesn’t exist.  В случае ошибок на этапе записи вывести сообщение Unable to save journal to file xx. Если курс не содержит задач, выводить сообщение No tasks added yet.

Пример команды:
Please, enter the command:
save journal 1

File name: d:/journal_java.txt
